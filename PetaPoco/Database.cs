...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
        ...
        public int Delete<T>(string sql, params object[] args)
        {
            ...
            return
                this.Execute(
                    string.Format("DELETE FROM {0} {1}", this._dbType.EscapeTableName(pd.TableInfo.TableName), sql),
                    args);
        }
        ...
        public int Delete<T>(Sql sql)
        {
            ...
            return
                Execute(
                    new Sql(string.Format("DELETE FROM {0}", this._dbType.EscapeTableName(pd.TableInfo.TableName)))
                        .Append(sql));
        }
        ...
        public bool Exists<T>(string sqlCondition, params object[] args)
        {
            ...
            return this.ExecuteScalar<int>(
                string.Format(this._dbType.GetExistsSql(), poco.TableName, sqlCondition), args) != 0;
        }
        ...
        public bool Exists<T>(object primaryKey)
        {
            return
                this.Exists<T>(
                    string.Format(
                        "{0}=:0", this._dbType.EscapeSqlIdentifier(PocoData.ForType(typeof(T)).TableInfo.PrimaryKey)),
                    primaryKey);
        }

        /// <summary>
        /// Checks for the existance of a row with the specified secondary key value.
        /// </summary>
        /// <typeparam name="T">The Type representing the table being queried</typeparam>
        /// <param name="secondaryKey">The primary key value to look for</param>
        /// <returns>True if a record with the specified primary key value exists.</returns>
        public bool Exists<T>(string secondaryKey)
        {
            return
                this.Exists<T>(
                    string.Format(
                        "{0}=:0", this._dbType.EscapeSqlIdentifier(PocoData.ForType(typeof(T)).TableInfo.SecondaryKey)),
                    secondaryKey);
        }
        ...
        {
            return this.Query(cb, sql, args).ToList();
        }
        ...
        {
            return this.Query(cb, sql.SQL, sql.Arguments).ToList();
        }
        ...
        public string FormatCommand(IDbCommand cmd)
        {
            return this.FormatCommand(
                cmd.CommandText, (from IDataParameter parameter in cmd.Parameters
                ...
                try
                {
                    using (var cmd = this.CreateCommand(this._sharedConnection, string.Empty))
                    {
                        ...
                        foreach (var i in pd.Columns)
                        {
                            ...
                            if (i.Value.ResultColumn || i.Value.OnlyUpdate)
                            {
                                continue;
                            }
                                ...
                                && string.Compare(i.Key, primaryKeyName, true) == 0)
                            {
                                ...
                                string autoIncExpression = this._dbType.GetAutoIncrementExpression(pd.TableInfo);
                            ...
                            names.Add(this._dbType.EscapeSqlIdentifier(i.Key));
                            values.Add(string.Format("{0}{1}", this._paramPrefix, index++));
                            this.AddParam(cmd, i.Value.GetValue(poco), i.Value.PropertyInfo);
                        }

                        string outputClause = string.Empty;
                        if (autoIncrement)
                        {
                            outputClause = this._dbType.GetInsertOutputClause(primaryKeyName);
                        }
                        ...
                        object id = this._dbType.ExecuteInsert(this, cmd, primaryKeyName);
            ...
            long page, long itemsPerPage, string sqlCount, object[] countArgs, string sqlPage, object[] pageArgs)
        {
            ...
            // Get the records
            result.Items = this.Fetch<T>(sqlPage, pageArgs);
        ...
        public IEnumerable<T> Query<T>(string sql, params object[] args)
        {
            ...
            try
            {
                using (var cmd = this.CreateCommand(this._sharedConnection, sql, args))
                {
                    ...
                    var factory =
                        pd.GetFactory(cmd.CommandText, this._sharedConnection.ConnectionString, 0, r.FieldCount, r) as
                        Func<IDataReader, T>;
                    using (r)
                    {
                        while (true)
        ...
        public IEnumerable<TRet> Query<TRet>(Type[] types, object cb, string sql, params object[] args)
        {
            ...
            try
            {
                using (var cmd = this.CreateCommand(this._sharedConnection, sql, args))
                {
                    ...
                    var factory = MultiPocoFactory.GetFactory<TRet>(
                        types, this._sharedConnection.ConnectionString, sql, r);
                    if (cb == null)
                    {
        ...
        public T Single<T>(object primaryKey)
        {
            return
                this.Single<T>(
                    string.Format(
                        "WHERE {0}=:0",
                        this._dbType.EscapeSqlIdentifier(PocoData.ForType(typeof(T)).TableInfo.PrimaryKey)),
                    primaryKey);
        }
        ...
        /// Throws an exception if there are zero or more than one record with the specified primary key value.
        /// </remarks>
        public T Single<T>(string secondaryKey)
        {
            return
                this.Single<T>(
                    string.Format(
                        "WHERE {0}=:0",
                        this._dbType.EscapeSqlIdentifier(PocoData.ForType(typeof(T)).TableInfo.SecondaryKey)),
                    secondaryKey);
        }
                    ...
                    using (var cmd = this.CreateCommand(this._sharedConnection, string.Empty))
                    {
                        ...
                        if (columns == null)
                        {
                            foreach (var i in pd.Columns)
                            {
                                ...
                                if (i.Value.ResultColumn || i.Value.OnlyInsert)
                                {
                                    continue;
                                }
                                ...
                                sb.AppendFormat(
                                    "{0} = {1}{2}", this._dbType.EscapeSqlIdentifier(i.Key), this._paramPrefix, index++);
                        ...
                        else
                        {
                            foreach (var colname in columns)
                            {
                                ...
                                sb.AppendFormat(
                                    "{0} = {1}{2}",
                                    this._dbType.EscapeSqlIdentifier(colname),
                                    this._paramPrefix,
                                    index++);

                                // Store the parameter in the command
                                this.AddParam(cmd, pc.GetValue(poco), pc.PropertyInfo);
                            }
                        ...
                        cmd.CommandText = string.Format(
                            ...
                            this._paramPrefix,
                            index++);
                        this.AddParam(cmd, primaryKeyValue, pkpi);
        ...
        public int Update<T>(string sql, params object[] args)
        {
            ...
            return
                this.Execute(
                    string.Format("UPDATE {0} {1}", this._dbType.EscapeTableName(pd.TableInfo.TableName), sql), args);
        }
        ...
        public int Update<T>(Sql sql)
        {
            ...
            return
                Execute(
                    new Sql(string.Format("UPDATE {0}", this._dbType.EscapeTableName(pd.TableInfo.TableName))).Append(
                        sql));
        }
        ...
        internal void DoPreExecute(IDbCommand cmd)
        {
            ...
            this._lastArgs = (from IDataParameter parameter in cmd.Parameters
                              select parameter.Value).ToArray();
        }
        ...
        private void CommonConstruct()
        {
            ...
            // Resolve the DB Type
            string DBTypeName =
                (this._factory == null ? this._sharedConnection.GetType() : this._factory.GetType()).Name;
            this._dbType = DatabaseType.Resolve(DBTypeName, this._providerName);
